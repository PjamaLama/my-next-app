...
export default function RecentActivity({ activity, activityError }: RecentActivityProps) {
  ...
  return (
    <section className="bg-[#18181b] rounded-xl shadow-md p-4 sm:p-6 border border-gray-800 mt-8 sm:mt-12">
      <div className="flex items-center justify-between mb-4">
        ...
        <svg 
          ...
          strokeWidth="2" 
          viewBox="0 0 24 24"
          className={`transition-transform duration-200 ${activitySectionExpanded ? 'rotate-180' : ''} text-gray-400 hover:text-gray-200 cursor-pointer`}
          ...
          {activity.length === 0 ? (
            ...
            <ul className="space-y-2 w-full">
                {activity.slice(0, 5).map((item, i) => {
                  ...
                  return (
                    <li key={`activity-item-${item.timestamp}-${i}`} className="flex flex-col gap-1 text-xs w-full p-2 sm:p-3 rounded-lg hover:bg-gray-800 transition">
                      <div className="flex items-start gap-2 sm:gap-3 cursor-pointer" onClick={() => setExpandedActivity(expanded ? null : i)}>
                  <span className="inline-flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gray-800 mt-0.5 flex-shrink-0">
                  ...
                  {item.entity === 'sheet' ? (
                    <>
                      <span className="font-medium text-gray-200">Sheet</span> <span className="capitalize">{item.type}</span> <span className="font-semibold text-white">{item.label}</span>
                    ...
                    <>
                      <span className="font-medium text-purple-300">Webhook</span> <span className="capitalize">{item.type}</span> <span className="font-semibold text-white">{item.label}</span>
                      {item.webhookType && (
                  ...
                  <span className="ml-2 text-gray-400">&middot; {dayjs(item.timestamp).fromNow()}</span>
                      ...
                      <svg 
                        ...
                        strokeWidth="2" 
                        viewBox="0 0 24 24"
                        className={`transition-transform duration-200 ${expanded ? 'rotate-180' : ''} text-gray-400 hover:text-gray-200 cursor-pointer flex-shrink-0`}
                      ...
                      <div className="mt-2 ml-7 sm:ml-9">
                        ...
                        {item.type === 'add' && item.entity === 'webhook' && (
                          <div className="mb-2 text-[11px] text-gray-400">
                            {item.sheetName && <span>Sheet: <span className="font-semibold text-emerald-300">{item.sheetName}</span></span>}
                            {item.rowNumber && <span className="ml-2">Row: <span className="font-semibold text-green-300">{item.rowNumber}</span></span>}
                          </div>
                        )}
                        {/* Show row data for webhook add */}
                        {item.type === 'add' && item.entity === 'webhook' && item.rowData && item.rowData.length > 0 && (
                          <div className="overflow-x-auto">
                            <table className="min-w-[200px] border border-gray-700 rounded text-xs">
                              <thead>
                                <tr>
                                  <th className="px-2 py-1 border-b border-gray-700 text-left">Column</th>
                                  <th className="px-2 py-1 border-b border-gray-700 text-left">Cell</th>
                                  <th className="px-2 py-1 border-b border-gray-700 text-left">Value</th>
                                </tr>
                              </thead>
                              <tbody>
                                {item.rowData.map((cell, idx) => (
                                  <tr key={`cell-${idx}-${cell.column}-${cell.cell}`}>
                                    <td className="px-2 py-1 border-b border-gray-800">{cell.column}</td>
                                    <td className="px-2 py-1 border-b border-gray-800">{cell.cell}</td>
                                    <td className="px-2 py-1 border-b border-gray-800">{cell.value}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
