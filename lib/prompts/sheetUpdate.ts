...
export function buildSheetUpdatePrompt(vars: SheetUpdatePromptVars): string {
  ...
  return [
    ...
    `For updates from files or text, infer all values (e.g., repeat patterns like Driver 'Neville Young', Vehicle 'TOYOTA HILUX', KM Start = previous KM End). If multi-sheet, generate actions per sheet.`,
