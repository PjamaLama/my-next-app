...
export async function handleExtractDataFromImages(args: any, context: Context, images: ImageData[], apiKey: string, res: NextApiResponse) {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        const flow = analyzeFileFlow(apiKey);
        const result = await flow.run({ prompt: transcript || 'Extract all relevant data from this file that could be added to a spreadsheet', files: [image] });
        analysisResults.push({ index: i + 1, type: image.mimeType, analysis: result, success: true });
      } catch (analysisError) {
    ...
    if (successfulAnalyses.length === 0) {
      return res.status(500).json({ success: false, error: 'Failed to analyze any files', details: 'All file analysis attempts failed. This may be due to AI service issues or unsupported file types.', analysisResults });
    }
    // If we found structured rows, surface a preview for confirmation via apply_structured_rows (dryRun)
    const extractedRows = analysisResults.map(r => (r.analysis as any)?.extracted_rows).filter(Boolean).flat();
    if (Array.isArray(extractedRows) && extractedRows.length > 0) {
      ...
      const toolCall = { function: { name: 'apply_structured_rows', arguments: JSON.stringify({ spreadsheetId, sheetName: selectedNames[0], rows: extractedRows, dryRun: true, commit: false }) } } as any;
      const execResp = await fetch(`${baseUrl}/api/genkit-tool-execute`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ toolCall, context: { spreadsheetId, sheetNames: selectedNames } }) });
      const execJson = await execResp.json();
      // Pass through preview so chat can render the proposed updates table
      return res.status(200).json({ success: true, result: `Preview ready for ${selectedNames[0]}.`, preview: execJson?.preview, details: { filesProcessed: images.length, successfulAnalyses: successfulAnalyses.length, analysisResults } });
    }
    // Fallback: minimal structured row via apply_structured_rows preview
    const sheets = await getGoogleSheetsClient();
    const escapedName = targetSheetName!.includes(' ') ? `'${targetSheetName!.replace(/'/g, "''")}'` : targetSheetName!;
    const headersResp = await sheets.spreadsheets.values.get({ spreadsheetId, range: `${escapedName}!A1:Z1`, valueRenderOption: 'FORMATTED_VALUE', dateTimeRenderOption: 'FORMATTED_STRING' });
    ...
    const execResp = await fetch(`${baseUrl}/api/genkit-tool-execute`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ toolCall, context: { spreadsheetId, sheetNames: [targetSheetName] } }) });
    const execJson = await execResp.json();
    return res.status(200).json({ success: true, result: `Preview ready for ${targetSheetName}.`, details: { filesProcessed: images.length, successfulAnalyses: successfulAnalyses.length, analysisResults, applyResult: execJson } });
  } catch (error) {
