...
Step 1: Analyze message and history for intent (summary/describe, update, query).
Step 2: If summary-like ("tell me about", "summarize", "what I did"), set intent="describe_data" and tools=[{"name":"describe_sheet","args":{${(context as any)?.sheetName ? `"sheetName":"${String((context as any).sheetName)}"` : ''}}}]. If the message mentions a sheet (e.g., "fuel weekly repo"), include that sheetName.
Step 3: If update-like ("add this data", "update with", "append"), set intent="update_data" and plan tool-calls as follows:
  - Step 1: Check sheetSummary/headers from context.
  - Step 2: If files present, chain extract_data_from_files to obtain structured rows.
  - Step 3: Compare extracted columns with headers. If compatible, plan apply_structured_rows with {rows: <extracted>, startRow: currentRows+1} depending on get_sheet_data result; else plan update_sheet with {transcript:${JSON.stringify(message)}, preview:true}.
  - A reasonable default chain is: [{toolName: 'get_sheet_data'}, ${hasFiles ? "{toolName:'extract_data_from_files'}," : ''} {toolName: 'apply_structured_rows', dependsOn: [0${hasFiles ? ',1' : ''}]}]. If unsure about compatibility, plan update_sheet instead of apply_structured_rows.
Step 4: Lower clarification threshold. Only ask to clarify if there is no accessible data (no headers/sheetSummary) and no useful history cues; otherwise include a best-guess tool.
