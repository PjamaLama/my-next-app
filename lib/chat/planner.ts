...
${fewShot}
  ...
  Step 4: For updates ("add", "update", "append"), set intent="update_data" and choose ONE of these paths:
    - For messages like "add to my sheet, client Stanley, sold 2000k":
      Step 1 (tabular): If context.sheetData has rows (tabular), parse fields (e.g., {"Client":"Stanley","Sales":2000}) and plan apply_structured_rows with params { rows: [parsed], startRow: (Array.isArray(context.sheetData?.[context.sheetName]) ? context.sheetData[context.sheetName].length + 1 : undefined) }.
      Step 2 (non-tabular): If context.isNonTabular, plan update_sheet with params { text: ${JSON.stringify(message)}, append: true }.
