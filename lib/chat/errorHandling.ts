...
export function handleHydrationError(e: any, context: any, sheetName: string) {
...
export function handleGenericHydrationError(e: any, context: any) {
...
export function createUserFriendlyError(context: any): boolean {
  try {
    if (typeof context?.error === 'string' && context.error.trim()) {
      const err = context.error as string;
      const name = String(context.sheetName || '').trim();
      let msg = '';
      if (err.includes('404')) {
      ...
      context._specificErrorResponse = msg;
      try {
        context._uiActions = context._uiActions || [];
        context._uiActions.push(
...
export function handleFatalError(e: any, context: Context) {
